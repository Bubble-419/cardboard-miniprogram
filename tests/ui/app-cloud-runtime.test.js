'use strict';

const test = require('node:test');
const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');
const vm = require('node:vm');

const projectRoot = path.resolve(__dirname, '../..');

test('资源方小程序使用自身云环境初始化，不经过跨应用共享鉴权', async () => {
  const source = fs.readFileSync(path.join(projectRoot, 'app.js'), 'utf8');
  const calls = { directInit: [], sharedCloud: [] };
  let appDefinition = null;

  function SharedCloud(options) {
    calls.sharedCloud.push(options);
    this.init = async () => {};
  }

  const wx = {
    cloud: {
      Cloud: SharedCloud,
      init(options) { calls.directInit.push(options); },
      callFunction() {},
      downloadFile() {}
    }
  };
  const context = {
    wx,
    App(definition) { appDefinition = definition; },
    require(request) {
      if (request === './utils/scanJoinGate') return { beginScanJoinFromLaunch() {} };
      throw new Error(`unexpected require: ${request}`);
    },
    console
  };

  vm.runInNewContext(source, context, { filename: 'app.js' });
  appDefinition.onLaunch.call(appDefinition, {});
  await appDefinition.globalData.cloudReady;

  assert.equal(calls.sharedCloud.length, 0);
  assert.equal(calls.directInit.length, 1);
  assert.equal(calls.directInit[0].env, 'cardboard-miniprogram-6a13aab073');
  assert.equal(calls.directInit[0].traceUser, true);
  assert.equal(appDefinition.globalData.cloud, wx.cloud);
});

test('自有环境配置不再保留共享环境分支且调试基础库一致', () => {
  const source = fs.readFileSync(path.join(projectRoot, 'app.js'), 'utf8');
  const projectConfig = JSON.parse(fs.readFileSync(path.join(projectRoot, 'project.config.json'), 'utf8'));
  const privateConfig = JSON.parse(fs.readFileSync(path.join(projectRoot, 'project.private.config.json'), 'utf8'));

  assert.equal(projectConfig.appid, 'wx6c484b4cfa055d73');
  assert.equal(projectConfig.libVersion, '3.12.1');
  assert.equal(privateConfig.cloudbaseCloudEnvId, 'cardboard-miniprogram-6a13aab073');
  assert.equal(privateConfig.libVersion, projectConfig.libVersion);
  assert.doesNotMatch(source, /new\s+wx\.cloud\.Cloud|resourceAppid|USE_SHARED_ENV|patchWxCloudForSharedEnv/);
  assert.equal(fs.existsSync(path.join(projectRoot, 'cloudfunctions/cloudbase_auth')), false);

  const cloudEntries = [
    'cloudfunctions/roomCommand/src/entry.js',
    'cloudfunctions/roomQuery/src/entry.js',
    'cloudfunctions/roomPresence/src/entry.js',
    'cloudfunctions/roomSignal/src/entry.js',
    'cloudfunctions/roomMedia/src/entry.js',
    'cloudfunctions/speechToText/src/entry.js',
    'cloudfunctions/saveInspiration/index.js',
    'cloudfunctions/getInspiration/index.js',
    'cloudfunctions/listInspirations/index.js'
  ];
  cloudEntries.forEach((relativePath) => {
    const entrySource = fs.readFileSync(path.join(projectRoot, relativePath), 'utf8');
    assert.doesNotMatch(entrySource, /FROM_OPENID|FROM_APPID|FROM_UNIONID/, relativePath);
  });
});
