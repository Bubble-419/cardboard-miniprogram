...
var require_room_cloudbase_adapter = __commonJS({
  "packages/room-cloudbase-adapter/index.js"(exports2, module2) {
    ...
    function createCloudBaseRoomRepository2(deps) {
      ...
      async function transactCommand(input, handler) {
        return db2.runTransaction(async (transaction) => {
          ...
          if (activeRoomId) {
            activeRoomDocument = await safeGet(transaction, COLLECTIONS2.rooms, activeRoomId);
          ...
          if (input.type === "CREATE_ROOM") {
            ...
            for (const candidate of input.roomIdCandidates || [input.roomId]) {
              if (!await safeGet(transaction, COLLECTIONS2.rooms, candidate)) {
      ...
      async function readSyncState(roomId, afterSeq, limit) {
        return db2.runTransaction(async (transaction) => {
          const room = await safeGet(transaction, COLLECTIONS2.rooms, roomId);
      ...
      async function findActiveRoom(userId) {
        return db2.runTransaction(async (transaction) => {
          ...
          const room = await safeGet(transaction, COLLECTIONS2.rooms, active.roomId);
    ...
    module2.exports = { COLLECTIONS: COLLECTIONS2, createCloudBaseRoomRepository: createCloudBaseRoomRepository2, digest, docId: docId2, safeGet };
  }
});
...
exports.main = async (event) => {
  ...
  if (!roomId || !sessionId || !turnId || signalType !== "PARTNER_SILENT_SOUND" || typeof rawValue !== "number" || !Number.isFinite(rawValue)) {
    return { ok: false, errCode: "INVALID_ARGUMENT", errMsg: "\u672A\u77E5\u77AC\u65F6\u4FE1\u53F7" };
  }
  try {
    const snapshot = await app.readSnapshot(roomId, {
      userId,
    ...
    const now = Date.now();
    if (!turn || !actor || session.sessionId !== sessionId || turn.turnId !== turnId || turn.activeMemberId !== actor.memberId || Number(turn.silentDeadlineAt) <= now) {
