    ...
    function createCloudBaseRoomRepository2(deps) {
      ...
      async function transactCommand(input, handler) {
        return db2.runTransaction(async (transaction) => {
          ...
          if (activeRoomId) {
            const activeRoom = await safeGet2(transaction, COLLECTIONS2.rooms, activeRoomId);
          ...
          if (decision.accepted) {
            ...
            if (decision.aggregate.currentSession) {
              ...
              await transaction.collection(COLLECTIONS2.sessions).doc(session.sessionId).set({ data: { ...session, roomId: resolvedRoomId } });
            }
            if (decision.archivedSession) {
              const archived = cleanDoc(decision.archivedSession);
              await transaction.collection(COLLECTIONS2.sessions).doc(archived.sessionId).set({ data: { ...archived, roomId: resolvedRoomId } });
            }
