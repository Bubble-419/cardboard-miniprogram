    ...
    function createCloudBaseRoomRepository2(deps) {
      ...
      async function transactCommand(input, handler) {
        return db2.runTransaction(async (transaction) => {
          ...
          const active = await safeGet2(transaction, COLLECTIONS2.active, docId2(input.actorUserId));
      ...
      async function upsertPresence({ roomId, memberId, deviceSessionId, lastSeenAt }) {
        ...
        return db2.runTransaction(async (transaction) => {
          const existing = await safeGet2(transaction, COLLECTIONS2.presence, presenceId);
          if (existing && Number(existing.lastSeenAt) >= Number(lastSeenAt)) return existing;
          await transaction.collection(COLLECTIONS2.presence).doc(presenceId).set({ data: row });
          return row;
        });
      }
      async function listPresence(roomId) {
        const result = await db2.collection(COLLECTIONS2.presence).where({ roomId }).orderBy("lastSeenAt", "desc").limit(50).get();
        return (result && result.data || []).map(cleanDoc);
      }
      async function listSignals(roomId) {
        const rows = await Promise.all(Object.values(SIGNAL_TYPES).map((signalType) => safeGet2(db2, COLLECTIONS2.signals, docId2(`${roomId}:${signalType}`))));
