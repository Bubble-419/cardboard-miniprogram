...
var require_room_cloudbase_adapter = __commonJS({
  "packages/room-cloudbase-adapter/index.js"(exports2, module2) {
    ...
    async function safeGet2(store, collection, id) {
      try {
    ...
    async function loadAggregate(store, roomId, preloadedRoom) {
      const room = preloadedRoom ? cleanDoc(preloadedRoom) : await safeGet2(store, COLLECTIONS2.rooms, roomId);
      if (!compatibleRoom(room)) return null;
      const currentSession = room.currentSessionId ? await safeGet2(store, COLLECTIONS2.sessions, room.currentSessionId) : null;
    ...
    async function loadSessionAggregate(store, roomId, sessionId) {
      const room = await safeGet2(store, COLLECTIONS2.rooms, roomId);
      if (!compatibleRoom(room)) return null;
      const currentSession = await safeGet2(store, COLLECTIONS2.sessions, sessionId);
    ...
    function publicSignalsId(roomId) {
      return docId2(`${roomId}:PUBLIC_SIGNALS`);
    }
    ...
    async function readPublicSignals(store, roomId) {
      const document = await safeGet2(store, COLLECTIONS2.signals, publicSignalsId(roomId));
    ...
    async function writePublicSignal(store, roomId, previousDocument, signalType, signal, now) {
      ...
      await store.collection(COLLECTIONS2.signals).doc(publicSignalsId(roomId)).set({ data: {
        roomId,
    ...
    function createCloudBaseRoomRepository(deps) {
      ...
      async function transactCommand(input, handler) {
        return db2.runTransaction(async (transaction) => {
          ...
          const existing = await safeGet2(transaction, COLLECTIONS2.actions, actionId);
          if (existing) {
          ...
          const active = requiresActiveLookup ? await safeGet2(transaction, COLLECTIONS2.active, docId2(input.actorUserId)) : null;
          let activeRoomId = active && active.roomId;
          let activeRoomDocument = null;
          let danglingActive = false;
          if (activeRoomId) {
            activeRoomDocument = await safeGet2(transaction, COLLECTIONS2.rooms, activeRoomId);
          ...
          if (input.type === COMMAND_TYPES.CREATE_ROOM) {
            ...
            for (const candidate of input.roomIdCandidates || [input.roomId]) {
              if (!await safeGet2(transaction, COLLECTIONS2.rooms, candidate)) {
          ...
          if (decision.accepted) {
            ...
            if (danglingActive && !actorWillHaveActiveIndex) {
              await transaction.collection(COLLECTIONS2.active).doc(docId2(input.actorUserId)).remove();
            }
            ...
            if (sessionPatch) {
              ...
              await transaction.collection(COLLECTIONS2.sessions).doc(afterSession.sessionId).set({ data: afterSession });
            }
            if (decision.archivedSession) {
              const archived = cleanDoc(decision.archivedSession);
              await transaction.collection(COLLECTIONS2.sessions).doc(archived.sessionId).set({ data: {
                ...archived,
                roomId: resolvedRoomId,
            ...
            for (const dirty of decision.dirtyFacts || []) {
              ...
              const factDocumentId = docId2(`${resolvedRoomId}:${dirty.kind}:${dirty.id}`);
              if (dirty.remove) {
                await transaction.collection(COLLECTIONS2.messages).doc(factDocumentId).remove();
                continue;
              }
              ...
              await transaction.collection(COLLECTIONS2.messages).doc(factDocumentId).set({ data });
            }
            for (const item of decision.events || []) {
              await transaction.collection(COLLECTIONS2.events).doc(`${resolvedRoomId}_${String(item.seq).padStart(12, "0")}`).set({ data: cleanDoc(item) });
            }
      ...
      async function readAggregate(roomId) {
        return db2.runTransaction(async (transaction) => {
          ...
          const first = await transaction.collection(COLLECTIONS2.events).where({ roomId }).orderBy("seq", "asc").limit(1).get();
      ...
      async function readSyncState(roomId, afterSeq, limit) {
        const room = await safeGet2(db2, COLLECTIONS2.rooms, roomId);
        ...
        const _ = db2.command;
        const result = await db2.collection(COLLECTIONS2.events).where({ roomId, seq: _.gt(afterSeq).and(_.lte(ceiling)) }).orderBy("seq", "asc").limit(limit).get();
      ...
      async function upsertPresence({ roomId, memberId, deviceSessionId, lastSeenAt }) {
        ...
        const presenceId = docId2(`${roomId}:${memberId}:${row.deviceSessionId}`);
        await db2.collection(COLLECTIONS2.presence).doc(presenceId).set({
      ...
      async function listPresence(roomId, cutoff) {
        ...
        const result = await db2.collection(COLLECTIONS2.presence).where(condition).orderBy("lastSeenAt", "desc").limit(50).get();
        return (result && result.data || []).map(cleanDoc);
      }
      ...
      async function upsertSignal(input) {
        return db2.runTransaction(async (transaction) => {
          const room = await safeGet2(transaction, COLLECTIONS2.rooms, input.roomId);
          ...
          if (input.signalType === SIGNAL_TYPES.DESIGN_PROBLEM_NUDGE) {
            ...
            const session = await safeGet2(transaction, COLLECTIONS2.sessions, input.sessionId);
...
function extractImageBuffer(response) {
  ...
  if (!buffer || !buffer.length || !(buffer[0] === 137 && buffer[1] === 80 || buffer[0] === 255 && buffer[1] === 216)) throw new Error("\u5C0F\u7A0B\u5E8F\u7801\u8FD4\u56DE\u5185\u5BB9\u65E0\u6548");
  return buffer;
}
async function generate(roomId) {
  const versions = [process.env.QR_ENV_VERSION, "develop", "trial", "release"].filter((value, index, list) => value && list.indexOf(value) === index);
  let lastError;
